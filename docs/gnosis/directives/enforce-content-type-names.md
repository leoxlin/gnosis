...
3. Replace built-in Concept Type names and the type references that resolve to them: `Gnosis Purpose`→`GnosisPurpose`, `Gnosis Process`→`GnosisProcess`, `Gnosis Directive`→`GnosisDirective`, and `Gnosis Decision`→`GnosisDecision`; update the corresponding runtime constant and tests.
