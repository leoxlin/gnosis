...
4. If a finding conflicts with an active decision, present the conflict to the author. If the author settles a replacement, persist a new `Gnosis Decision` with `type`, `title`, `description`, `# Decision`, `# Why`, optional `# Constraints`, and a `supersedes` link.
