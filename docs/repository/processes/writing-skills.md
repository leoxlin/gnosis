...
Writing skills applies red-green-refactor to agent behavior. Gnosis Process pages hold repository-owned workflow knowledge; packaged skills provide only the invocation and execution adapter that must exist at runtime.

## Use when

- Creating or changing a packaged skill.
- Moving reusable workflow detail from a skill into a Gnosis Process record.
...
4. **Green:** Write the smallest instruction change that corrects the observed behavior. Keep ordered runtime steps in the skill and detailed repository workflow knowledge in the Gnosis Process page, with one explicit context pointer from the skill.
