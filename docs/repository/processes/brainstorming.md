...
1. Read the knowledge inputs and map discoverable facts before asking the author. If the request conflicts with or changes repository purpose, use `refine-purpose` before designing.
...
7. For every settled choice, apply the Repository Decision boundary. Invoke `record-decision` only for a non-obvious durable choice whose rationale or constraints must outlive this task. Leave routine design details for the directive.
8. Invoke [writing-plans](writing-plans.md). Do not write a separate specification document.

## Completion

The author has approved a coherent design, every qualifying durable decision has been recorded through `record-decision`, and `writing-plans` has begun translating the design into a directive. No implementation has started.
