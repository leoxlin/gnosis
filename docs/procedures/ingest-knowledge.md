...
1. List exact types with `gnosis get concepts`, then read only the applicable Concept Type definitions with `gnosis get pages '<concept-type URI>' --full`. Read candidate identity matches returned by `gnosis search knowledge --backend lexical --vault <name> '<identity question>'`.
...
3. Integrate by identity. Update matching pages and create only the smallest useful set of new records. When no existing type fits, invoke `gnosis get procedures 'gnosis://_/procedures/create-concept-type.md' --full` and resume after that procedure completes.
...
6. When `vault_index` is enabled, run `gnosis index vault --vault <name>`. Run `gnosis validate vault --vault <name>` in every case.
