...
3. Verify each purpose/decision finding and record `F<n> → ACCEPT|REJECT|AUTHOR` with evidence. A reviewer cannot settle intent: ask the author about choices absent from approved requirements. Invoke the **Create or update** branch of [manage-purpose](manage-purpose.md) for accepted purpose changes. Invoke the matching **Create** or **Update** branch of [manage-decisions](manage-decisions.md) for accepted decision creation or changes. Persist semantic decision changes as new superseding Decisions; edit in place only for non-semantic corrections.
