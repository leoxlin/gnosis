...
2. Find candidates according to the type's identity rules. Query before reading non-singleton records with an identity-focused `gnosis search knowledge --backend lexical --vault <root> '<question>'`, then read only applicable `should_read` records. List singleton records directly. Read an exact target as JSON to retain its URI, revision, origin, status, and unknown metadata. Stop on an identity conflict.
