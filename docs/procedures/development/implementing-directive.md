...
6. When a failure's root cause is not established, invoke `gnosis procedure invoke --uri 'gnosis://core/procedures/debugging/systematic-debugging.md'` and vary one evidenced hypothesis at a time. Apply the resulting fix only to the selected directive and current task.
