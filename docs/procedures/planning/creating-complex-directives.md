...
4. Self-review coverage, names/types, dependency order, DRY, YAGNI, test quality, task size, and evidence. Persist every `draft` with `gnosis write '<directive URI>' --filename <draft-file>`; bind dependency links/revisions/contracts, then read back each URI/revision with `gnosis read '<directive URI>' --json`.
