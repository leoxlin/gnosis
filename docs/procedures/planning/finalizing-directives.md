...
3. Apply accepted feedback. If a simple draft becomes complex or directive boundaries/count change, return the affected set to [creating-directives](creating-directives.md). After any other material correction, persist the `draft`, read its new revision, then repeat [reviewing-directives](reviewing-directives.md). Stop for the author if the same accepted blocker survives one correction.
