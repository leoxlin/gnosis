...
4. For diagnosis or planning, return the evidence-backed cause, regression boundary, constraints, and reproduction without changing production behavior. Otherwise, create the smallest failing reproduction and invoke `gnosis procedure invoke --uri 'gnosis://core/procedures/execution/test-driven-development.md' --pretty`; implement one source-level fix and verify it.
5. After three failed fix attempts, stop and question the architecture with the author. Invoke `gnosis procedure invoke --uri 'gnosis://core/procedures/planning/refining-requirements.md' --pretty` with the existing evidence before changing the design.
