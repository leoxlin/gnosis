...
1. Resolve the vault and its link format. List candidate targets with `gnosis get pages --vault <name> --fields uri,title` and collect each page's `aliases` on demand.
...
8. Run `gnosis validate vault --vault <name>` and report inserted links, added relationships, and skipped low-confidence mentions.
