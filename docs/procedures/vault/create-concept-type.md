...
1. Resolve the vault and read its configuration and agent rules. List existing types with `gnosis get concepts --json` and inspect only relevant definitions with `gnosis read '<concept-type URI>'`.
...
6. Run `gnosis index vault --vault <root>` only when `vault_index` is enabled, then run `gnosis validate --vault <root>`.
