...
5. Retrieve the nearest existing memories for each remaining candidate with `gnosis search knowledge --backend lexical --vault <name> "<statement>"`, keeping only Memory records; add `gnosis search knowledge --backend vector --vault <name> "<statement>"` when semantic retrieval is configured.
...
8. Run `gnosis validate vault --vault <name>`. Report the per-candidate operations and evidence.
