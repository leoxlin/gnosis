...
5. Run `gnosis index vault --vault <name>` when indexing is enabled, then run `gnosis validate vault --vault <name>` and every repository check that directly governs the changed records. Inspect complete output, warnings, exit status, and the final diff.
