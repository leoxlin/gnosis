...
2. Run `gnosis search knowledge --backend lexical --vault <name> "<question>"` and keep only Memory records; add `gnosis search knowledge --backend vector --vault <name> "<question>"` when semantic retrieval is configured, merging candidates by URI.
