...
5. Write `docs/concepts/<kebab-name>.md` with `type: Concept Type`, `title`, and `description`. Add other frontmatter only when a consumer actually uses it.
...
8. Regenerate indexes only when `vault_index` is enabled, then validate the vault before handoff.
