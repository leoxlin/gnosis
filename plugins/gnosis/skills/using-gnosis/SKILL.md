---
name: using-gnosis
description: Select and apply canonical vault Procedure records for work in a gnosis vault.
---
...
2. Discover Procedures with `get_procedures` and tags `gnosis,vault` exactly once for the task when the host exposes that MCP tool; otherwise run `gnosis get procedures --tags gnosis,vault` exactly once. Do not delegate discovery or selection to a sub-agent.
...
5. Read each selected procedure in dependency order with `get_procedures` and its exact URI when exposed; otherwise run `gnosis get procedures 'gnosis://....' --full`. Use the exact URI returned by discovery.
6. Follow a single-step procedure's `Inputs`, `Process`, and `Completion` sections, or follow a multi-step procedure's numbered `STEP` sections according to their routing and branch instructions. Use gnosis page reads, knowledge queries, and `trace_graph` when those MCP tools are exposed; otherwise use the matching CLI operations. Select another procedure only when the active contract requires it, and bind that selection to its exact URI.
