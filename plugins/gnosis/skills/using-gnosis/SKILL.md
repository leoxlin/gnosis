...
2. Run `gnosis get procedures --tags gnosis,vault` exactly once for the task. Do not delegate discovery or selection to a sub-agent.
...
5. Read each selected procedure in dependency order with `gnosis get procedures 'gnosis://....' --full`, using the exact URI returned by discovery.
