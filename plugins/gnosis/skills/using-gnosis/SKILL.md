...
2. For every task, dispatch a fresh read-only selector sub-agent with the author's exact request, the resolved vault, and the applicable agent instructions. Require the selector to:
   ...
   - When the skill was invoked as `using-gnosis debug`, also return a concise summary of the selected processes as runnable `gnosis` commands, in dependency order (for example, `gnosis process invoke --id '<gnosis URI>' --pretty`).
